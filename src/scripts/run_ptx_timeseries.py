...
from build_ptx_cases import (SCENARIOS, COMP_FRACTION, COMP_SCENARIOS, build_net,
                             A_PD, B_PD, G_PG, P_ELYS_MW, P_SOLAR_MW)

LOG = [("res_bus", "vm_pu"), ("res_bus", "va_degree"),
       ("res_line", "loading_percent"), ("res_line", "pl_mw"),
       ("res_ext_grid", "p_mw"), ("res_ext_grid", "q_mvar"),
       ("res_gen", "p_mw"), ("res_gen", "q_mvar")]
...
def run_scenario(name, p_coord, prof, outdir):
    ...
    load_p = np.where(prof["ptx"] >= 0, prof["ptx"] * P_ELYS_MW,
                      prof["ptx"] * P_SOLAR_MW)
    df = pd.DataFrame({"cons": prof["consumption"], "loadP": load_p,
                       "genG": prof["generator"] * G_PG})
    ds = DFData(df)
    n = len(df)

    ConstControl(net, "load", "scaling", element_index=[a, b],
                 data_source=ds, profile_name=["cons", "cons"])
    ConstControl(net, "load", "p_mw", element_index=[p],
                 data_source=ds, profile_name="loadP")
    ConstControl(net, "gen", "p_mw", element_index=[g],
                 data_source=ds, profile_name="genG")

    os.makedirs(outdir, exist_ok=True)
    pd.DataFrame({"hour": np.arange(n),
                  "A_mw": prof["consumption"] * A_PD,
                  "B_mw": prof["consumption"] * B_PD,
                  "P_mw": load_p, "G_mw": prof["generator"] * G_PG}
                 ).to_csv(os.path.join(outdir, "inputs.csv"), index=False)

    ow = OutputWriter(net, time_steps=range(n), output_path=outdir,
                      output_file_type=".csv")
    for tbl, var in LOG:
        ow.log_variable(tbl, var)

    run_timeseries(net, time_steps=range(n), continue_on_divergence=True,
                   verbose=False)
    ...
    return dict(scenario=name, steps=n, diverged=diverged,
                vm_min=np.nanmin(vm.values), vm_max=np.nanmax(vm.values),
...
def main():
    ...
    for name, pc in SCENARIOS.items():
        ...
        rows.append(s)
        print("%s  diverged %3d/%d  Vm[%.3f,%.3f]  maxLine %4.0f%%  slack[%+.0f,%+.0f] MW"
              % (s["scenario"], s["diverged"], s["steps"], s["vm_min"],
                 s["vm_max"], s["max_line_load"], s["slack_min_mw"],
                 s["slack_max_mw"]))
