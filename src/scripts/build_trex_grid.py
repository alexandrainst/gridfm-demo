...
def build():
    ...
    slack = next(i for i in np.argsort(-ys)          # highest back, not a PtX
    ...
    D = np.hypot(coords[:, 0, None] - coords[None, :, 0],
                 coords[:, 1, None] - coords[None, :, 1])
    ...
    bus = [pp.create_bus(net, vn_kv=V_KV, max_vm_pu=1.1, min_vm_pu=0.9,
    ...
    pp.create_ext_grid(net, bus[slack], vm_pu=1.02, va_degree=0.0,
                       max_p_mw=9999, min_p_mw=-9999,
                       max_q_mvar=9999, min_q_mvar=-9999)
    pg = 0.75 * total_load / N_GEN
    for i in gens:
        pp.create_gen(net, bus[i], p_mw=pg, vm_pu=1.01,
                      max_q_mvar=0.6 * pg, min_q_mvar=-0.6 * pg,
                      max_p_mw=1.5 * pg, min_p_mw=0.0)
    ...
    for e in edges:
        ...
        pp.create_line_from_parameters(net, bus[a], bus[b], length_km=float(L),
                                       r_ohm_per_km=R_PER_KM, x_ohm_per_km=X_PER_KM,
                                       c_nf_per_km=C_PER_KM_NF, max_i_ka=MAX_I_KA)
    ...
    info = dict(coords=coords, role=role, edges=edges, eye=eye_idx,
                slack=slack, gens=gens, ptx=ptx, n=n,
                total_load=total_load, conv=conv, net=net)
    if conv:
...
def plot(info, outpng):
    ...
    for kind, (m, c, _) in STYLE.items():
        ...
        if idx:
            ax.scatter(coords[idx, 0], coords[idx, 1], marker=m, c=c, s=70,
                       edgecolors="black", linewidths=0.6, zorder=3)

    # highlight the eye PtX
    ex, ey = coords[info["eye"]]
    ax.scatter([ex], [ey], marker="D", s=260, facecolors="none",
               edgecolors="#9467BD", linewidths=2.0, zorder=4)
    ax.annotate("PtX in the eye", (ex, ey), xytext=(ex - 24, ey + 9),
                fontsize=10, color="#6a3da5", fontweight="bold",
                arrowprops=dict(arrowstyle="->", color="#6a3da5"))

    if info["conv"]:
        sub = ("converged | Vm[%.3f, %.3f] | max line %.0f%% | slack %+.0f MW | "
               "loss %.0f MW | load %.0f MW"
               % (info["vmin"], info["vmax"], info["maxload"],
                  info["slack_p"], info["loss"], info["total_load"]))
    else:
        sub = "DID NOT CONVERGE"
    ax.set_title("Jurassic Grid — 200-bus T-Rex network\n" + sub, fontsize=13,
                 fontweight="bold")
    ax.set_aspect("equal")
    ax.axis("off")
...
def main():
    ...
    net, info = build()
    print("buses=%d edges=%d gens=%d ptx=%d slack=1  conv=%s"
          % (info["n"], len(info["edges"]), len(info["gens"]),
             len(info["ptx"]), info["conv"]))
    if info["conv"]:
        print("Vm[%.3f, %.3f]  maxLoad %.0f%%  slack %+.0f MW  loss %.0f MW  load %.0f MW"
              % (info["vmin"], info["vmax"], info["maxload"],
                 info["slack_p"], info["loss"], info["total_load"]))
