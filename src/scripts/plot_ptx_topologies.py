...
def parse_case(path):
    ...
    return dict(name=os.path.basename(path).replace("case_ptx_", "").replace(".m", ""),
                coords=coords, order=order, status=status, base=base,
                bus=matrix("bus"), gen=matrix("gen"), branch=matrix("branch"),
                kv=matrix("bus")[0, 9])
...
def add_shunt_glyph(ax, x, y, bs_mvar):
    ...
    if bs_mvar < 0:                                                # reactor coil
        ...
        da.add_artist(Line2D(13 + 3 * np.sin(t), 14 + t / (3 * np.pi) * 16,
                             color=col, lw=1.3))
    ...
    ab = AnnotationBbox(da, (x, y), xybox=(30, -16), xycoords="data",
                        boxcoords="offset points", frameon=False, pad=0,
                        box_alignment=(0.5, 1.0),
                        arrowprops=dict(arrowstyle="-", color=col, lw=1.2))
    ax.add_artist(ab)
    ax.annotate(label, (x, y), xytext=(46, -22), textcoords="offset points",
                fontsize=6, color=col, va="top", zorder=5)
...
def plot_case(ax, case):
    ...
    for r in case["branch"]:
        ...
        ax.annotate("%.0f km" % L, ((x0 + x1) / 2, (y0 + y1) / 2),
                    fontsize=6.5, color="0.35", ha="center", va="center",
    ...
    for bus_i, nm in name_by_idx.items():
        ...
        x, y = pos[nm]
        ax.scatter([x], [y], marker=marker, s=190, c=color,
    ...
    ax.set_title("%s   (%.0f kV, %.0f MVA base)\n%s"
                 % (case["name"].upper(), case["kv"], case["base"], case["status"]),
                 fontsize=8.5, color="#2a6e2a" if ok else "#a01010")
...
def main():
    ...
    fig.suptitle("Danish PtX siting demo — network topologies (move plant P)",
