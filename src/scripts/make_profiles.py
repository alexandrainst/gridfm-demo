...
def save(prof, outdir):
    ...
    rows = np.column_stack(
        [prof["t_h"], prof["t_d"], prof["consumption"], prof["ptx"], prof["generator"]]
    )
    np.savetxt(
        os.path.join(outdir, "profiles_30day.csv"),
        rows,
        delimiter=",",
        header="hour,day,consumption,ptx,generator",
        comments="",
    )
...
def plot(prof, outpng):
    ...
    ax[0].set_title(
        "Consumption — winter seasonal envelope × daily shape (peaks 08:00 & 17:00)"
    )
    ...
    fig.suptitle(
        "30-day hourly profiles — simple 5-bus PtX network",
        fontsize=14,
        fontweight="bold",
    )
...
def main():
    ...
    print(
        "arrays: %d steps each | consumption[min,max]=[%.2f,%.2f] "
        "ptx[min,max]=[%.2f,%.2f] gen∈{%.2f,%.2f}"
        % (
            N,
            prof["consumption"].min(),
            prof["consumption"].max(),
            prof["ptx"].min(),
            prof["ptx"].max(),
            GEN_LO,
            GEN_HI,
